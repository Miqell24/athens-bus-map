...
async function init() {
  ...
  paintMeta();
  // tram keys carry the GTFS T prefix (T6/T7) but the city writes bare 6/7 —
  // strip it from every displayed text, never from the key (trolleybus 6 exists)
  // Proastiakos carries no number in the city (user 17.09.2026): the panel,
  // the popups and the planner name the route instead of A1–A4
  const PROAST = { A1: 'Piraeus – Airport', A2: 'Ano Liosia – Airport', A3: 'Athens – Chalkida', A4: 'Piraeus – Kiato' };
  const dispLine = (l) => PROAST[l] || (/^T\d+$/.test(l) ? l.slice(1) : l);
  const paintChips = () => {
    document.getElementById('chips').innerHTML = meta.lines
      .map((l) => `<button class="chip${l.h24 ? ' h24' : ''}" data-line="${esc(l.line)}" style="background:${esc(l.color)}">${esc(dispLine(l.line))}${l.h24 ? '<span class="plus">+</span>' : ''}</button>`)
      .join(' ');
  };
  paintChips();
  ...
  const addBadgeLayer = ([z0, z1], b) => {
    ...
    map.addLayer({
      ...
      layout: {
        'text-field': ['case', isH24, ['format', ['coalesce', ['get', 'disp'], ['get', 'line']], {}, ' +', { 'text-color': '#000000' }], ['coalesce', ['get', 'disp'], ['get', 'line']]],
        'text-font': [NARROW_BOLD],
  ...
  // single line ("Skoufa Σκουφά"). Our labels put the local name first with the
  ...
  const state = { bus: true, tram: true, metro: true, selected: null, journey: null };
  ...
  function applyFilters() {
    ...
    // METRO AND TRAM ARE TWO CATEGORIES. They share the rail mode in the data —
    // both ride tracks, both are matched on the rail graph — but the reader sees
    // three networks, so each arm follows its own toggle. The split is the
    // metro:1 flag the pipeline puts on M-lines (user report: the tram sat in
    // the metro's category and wore the metro's colour).
    const railC = ['any',
      state.tram ? ['all', ['==', ['get', 'mode'], 'tram'], ['!=', ['get', 'metro'], 1]] : false,
      state.metro ? ['all', ['==', ['get', 'mode'], 'tram'], ['==', ['get', 'metro'], 1]] : false];
    const modeC = ['any', state.bus ? ['==', ['get', 'mode'], 'bus'] : false, railC];
    ...
    let numC, numField;
    if (state.bus && !state.tram && !state.metro) {
  ...
  for (const [id, key] of [['toggle-bus', 'bus'], ['toggle-tram', 'tram'], ['toggle-metro', 'metro']]) {
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `athens-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  map.on('click', 'stops-dots', (e) => {
    ...
    stopPopup = new maplibregl.Popup({ closeButton: false, offset: 10 })
      ...
      .setHTML(`<strong>${esc(p.name)}</strong>${p.terminus ? ' · terminus' : ''}<br>${label}: ${esc(p.lines.split(', ').map((n) => dispLine(n) + (H24_KEYS.includes(p.mode + '|' + n) ? ' +' : '')).join(', '))}`)
      .addTo(map);
  });
  ...
  // NOT fitBounds(meta.bbox): the OASA network reaches Elefsina, Rafina and Kapandriti, so fitting the data
  // bbox would open on countryside with the city as a blob. Open on the city — the
  // whole network is one zoom out.
  map.jumpTo({ center: [23.727, 37.983], zoom: 11.3 });
}
